...
// TestArchiveOldHistory_Integration tests archiving old price history
func TestArchiveOldHistory_Integration(t *testing.T) {
	db := getTestDB(t)
	if db == nil {
		return
	}
	...
	// Create a test IPO
	testIPOID := uuid.New().String()
	testCompanyCode := "TESTIPO3"

	_, err := db.Exec(`
		INSERT INTO ipo_list (id, stock_id, name, company_code, registrar, status)
		VALUES ($1, $2, $3, $4, $5, $6)
	`, testIPOID, "TEST003", "Test IPO 3 Ltd", testCompanyCode, "Test Registrar", "CLOSED")
	if err != nil {
		t.Fatalf("Failed to create test IPO: %v", err)
	}
	defer func() {
		db.Exec("DELETE FROM ipo_list WHERE id = $1", testIPOID)
		db.Exec("DELETE FROM gmp_price_history_archive WHERE ipo_id = $1", testIPOID)
	}()
	defer cleanupTestDataByIPO(t, db, testIPOID)

	// Insert old and recent price history data
	now := time.Now()
	...
	`, uuid.New().String(), testIPOID, testCompanyCode, oldDate, 100.0, 10.0,
		110.0, 10.0, 10.0, "subscribed", 5.0, "test", "investorgain.com")
	if err != nil {
	...
	`, uuid.New().String(), testIPOID, testCompanyCode, recentDate, 100.0, 15.0,
		115.0, 15.0, 15.0, "subscribed", 7.5, "test", "investorgain.com")
	if err != nil {
	...
	if err != nil {
		t.Fatalf("Failed to archive old history: %v", err)
	}

	if report.RecordsArchived != 1 {
		t.Errorf("Expected 1 entry to be archived, got %d", report.RecordsArchived)
	}
	...
	// Verify old entry was removed from main table
	var mainCount int
	err = db.QueryRow("SELECT COUNT(*) FROM gmp_price_history WHERE ipo_id = $1", testIPOID).Scan(&mainCount)
	if err != nil {
		t.Fatalf("Failed to count main table entries: %v", err)
	}

	if mainCount != 1 {
		t.Errorf("Expected 1 entry remaining in main table, got %d", mainCount)
	}

	// Verify old entry exists in archive table
	var archiveCount int
	err = db.QueryRow("SELECT COUNT(*) FROM gmp_price_history_archive WHERE ipo_id = $1", testIPOID).Scan(&archiveCount)
	if err != nil {
		t.Fatalf("Failed to count archive table entries: %v", err)
	}

	if archiveCount != 1 {
		t.Errorf("Expected 1 entry in archive table, got %d", archiveCount)
	}
}
